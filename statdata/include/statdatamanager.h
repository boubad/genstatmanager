...
public:
		...
		if (pStmtFetch == nullptr) {
			...
			this->m_pstatementFetch.reset(
					new sqlite::Statement(pBase,
							SQL_VALUES_BY_VARIABLE_NOT_NULL));
	...
	bool get_variable_values_pair(int nVarId,
			std::map<int, boost::any, COMPARE, ALLOCPAIR> &oMap, int skip,
			int taken) {
		oMap.clear();
		if ((skip < 0) || (taken < 1)) {
			return (false);
		...
		if (pStmtFetch == nullptr) {
			...
			this->m_pstatementFetch.reset(
					new sqlite::Statement(pBase,
							SQL_VALUES_PAIR_BY_VARIABLE_NOT_NULL));
			pStmtFetch = this->m_pstatementFetch.get();
			if (pStmtFetch == nullptr) {
				return (false);
			}
		...
		while (pStmtFetch->has_values()) {
			...
			}
			{
				...
				if (pStmtFetch->col_value(1, v)) {
					...
					intra::Value x;
					x.value(vx);
					this->convert_value(x, vartype, aval);
				}
			}
			if ((nIndivId != 0) && (!aval.empty())) {
				oMap[nIndivId] = aval;
			}
			if (!pStmtFetch->next()) {
				this->m_pstatementFetch.reset();
		...
		return (true);
	} // get_variable_values_pair
	template <class ALLOCINT, class ALLOCANY, class ALLOCANYVEC>
	bool get_variables_values_pair(const std::vector<int,ALLOCINT> &oVarsIds,
			std::vector<int,ALLOCINT> &oIndsIds, std::vector<std::vector<boost::any,ALLOCANY>,ALLOCANYVEC> &oVals) {
		oIndsIds.clear();
		oVals.clear();
		if (oVarsIds.empty()){
			return (false);
		}
		auto iend = oVarsIds.end();
		for (auto it = oVarsIds.begin(); it != iend; ++it){
			if (*it == 0){
				return (false);
			}
		}// it
		std::set<int,std::less<int>,ALLOCINT> oSet;
		std::map<int,std::map<int,boost::any> > oWorkMap;
		const int taken = 100;
		for (auto it = oVarsIds.begin(); it != iend; ++it){
			int nVarId = *it;
			if (nVarId == 0){
			...
			int skip = 0;
			int nCount = 0;
			if (!this->get_variable_values_count(nVarId,nCount)){
				continue;
			}
			if (nCount < 1){
				continue;
			...
			while (skip < nCount){
				std::map<int,boost::any> xMap;
				if (!this->get_variable_values_pair(nVarId,xMap, skip,taken)){
					break;
				}
				if (xMap.empty()){
					break;
				...
				if (oSet.empty()){
					...
					for (auto jt = xMap.begin(); jt != jend; ++jt){
						...
						oSet.insert(nId);
						std::map<int,boost::any> zMap;
						zMap[nVarId] = v;
						oWorkMap[nId] = zMap;
					}// jt
				} else {
					std::set<int,std::less<int>,ALLOCINT> oSetDelete;
					auto jend = oSet.end();
					auto kend = xMap.end();
					for (auto jt = oSet.begin(); jt != jend; ++jt){
						int nId = *jt;
						auto kx = xMap.find(nId);
						if (kx == kend){
							oSetDelete.insert(nId);
						} else {
							std::map<int,boost::any>  &ymap = oWorkMap[nId];
							boost::any v = xMap[nId];
							ymap[nVarId] = v;
						}
					}// jt
					std::for_each(oSetDelete.begin(),oSetDelete.end(),[&](int nId){
						auto lt = oSet.find(nId);
						if (lt != oSet.end()){
							oSet.erase(lt);
						}
					});
		...
		//assert(oWorkMap.size() == n);
		auto indend = oSet.end();
		oIndsIds.resize(n);
		oVals.resize(n);
		const size_t nCols = oVarsIds.size();
		size_t ii = 0;
		for (auto it = oSet.begin(); it != indend; ++it){
			std::vector<boost::any,ALLOCANY> cur(nCols);
			int nId = *it;
			oIndsIds[ii] = nId;
			std::map<int,boost::any>  &ymap = oWorkMap[nId];
			auto yyend = ymap.end();
			for (size_t j = 0; j < nCols; ++j){
				int nVarId = oVarsIds[j];
				if (ymap.find(nVarId) != yyend){
					boost::any v = ymap[nVarId];
					cur[j] = v;
				}
			}// j
			oVals[ii++] = cur;
...
bool StatDataManager<TSTRING>::get_dataset_indivs_by_status_count(int nId,
		const typename StatDataManager<TSTRING>::StringType &status,
		int &nCount) {
	assert(this->is_valid());
	nCount = 0;
	sqlite::Database *pBase = this->m_database.get();
...
void StatDataManager<TSTRING>::convert_value(
		...
		const typename StatDataManager<TSTRING>::StringType &vartype,
		boost::any &v) {
