...
namespace intrasqlite {
	...
	private:
		// no implementation
		StatDataManager(const StatDataManager &other);
		StatDataManager & operator=(const StatDataManager &other);
	private:
	...
	public:
		...
		bool process_data(const std::string &srcfilename,
			const std::string &datasetSigle,
			DatasetType &oSet, const char & delim = char('\t'),
			const std::string & na = std::string("na"));
		bool process_data(const std::string &srcfilename,
			const std::string &datasetSigle,
...
#else
		bool process_data(const std::wstring &srcfilename,
			const std::wstring &datasetSigle,
			DatasetType &oSet, const wchar_t & delim = wchar_t(L'\t'),
			const std::wstring & na = std::wstring(L"na"));
		bool process_data(const std::wstring &srcfilename,
			const std::wstring &datasetSigle,
			const wchar_t &delim = wchar_t(L'\t'),
			const std::wstring &na = std::wstring(L"na"));
		bool open(const std::wstring &wfilename);
#endif // INTRA_USE_WSTRING
		bool fill_data(const ImportDataType &oData, DatasetType &oSet);
	...
	public:
		...
		bool get_variable_values(int nVarId,
		...
		bool maintains_indivs(const std::vector<intra::Indiv, ALLOCIND> &oVec) {
			...
			for (auto it = oVec.begin(); it != oVec.end(); ++it) {
				...
				if ((nId == 0) && (nDatasetId != 0)) {
					intra::String ss = boost::to_upper_copy(boost::trim_copy(sigle));
					stmtFetch2.reset();
